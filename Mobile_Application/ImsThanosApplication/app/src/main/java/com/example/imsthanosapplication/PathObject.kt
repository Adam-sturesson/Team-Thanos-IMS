...
object PathObject {
    ...
    fun drawPath(bitmap: Bitmap) {
        ...
        for (i in 0..PathCompanion.listOfPoints.size - 2) {
            path.moveTo(
                (PathCompanion.listOfPoints[i].x.toFloat() * 3) + (width / 2),
                (PathCompanion.listOfPoints[i].y.toFloat() * 3) + (height / 2)
            )
            path.lineTo(
                (PathCompanion.listOfPoints[i + 1].x.toFloat() * 3) + (width / 2),
                (PathCompanion.listOfPoints[i + 1].y.toFloat() * 3) + (height / 2)
            )
        }
