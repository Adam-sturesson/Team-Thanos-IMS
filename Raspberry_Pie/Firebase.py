...
class MovementData:
    def __init__(self, previousPosition, distanceFromPrevPos, angle):
...
def receiveDataFromArduino():
    movementData = MovementData(Position(0,0), 1, 45)
    return movementData
    
def convertMovementDataToPosition (movementData):
...
# Arduino commands for start and stop routes
startRouteCommand = "startRoute"
stopRouteCommand = "stopRoute"
...
while True:
    ...
    try :
        dataFromArduino = receiveDataFromArduino() # This should wait for data 
        
        if dataFromArduino == startRouteCommand:
            routeStartTime = time.asctime(time.localtime(time.time()))
        
        elif dataFromArduino == stopRouteCommand:
            addRouteInDb()
        
        else : 
            currentPosition = convertMovementDataToPosition(dataFromArduino)
